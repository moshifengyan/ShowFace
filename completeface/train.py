...
parser.add_argument("-ns", "--nsnapshot", type=int, default=1800)
...
writer = SummaryWriter(log_dir=os.path.join(os.path.dirname(__file__),"/logs"))
# Initialize networks
def weights_init(m):
...
print(args)
modelf_path = os.path.join(os.path.dirname(__file__),args.modelf)
outf_path = os.path.join(base_path,args.outf)
try:
    os.makedirs(outf_path)
except OSError:
    pass
...
train_loader = torch.utils.data.DataLoader(
    dataset=data_loader.ImageList( train_list, transform=transforms.Compose([
        transforms.ToTensor(),
...
load_model(G_xvz, modelf_path, 'netG_xvz_epoch_4_1799.pth')
load_model(G_vzx, modelf_path, 'netG_vzx_epoch_4_1799.pth')
load_model(D_xvs, modelf_path, 'netD_xvs_epoch_4_1799.pth')
    ...
    for epoch in range(args.epochs):
        ...
        # for i, data in enumerate(train_loader):
            ...
            grads = autograd.grad(outputs = D_x_hat_s,
                                  ...
                                  # grad_outputs=torch.ones(D_x_hat_s.size()),
                                  retain_graph = True,
                                  create_graph = True,
                                  only_inputs = True)[0]
            ...
            if i % snapshot == snapshot-1:
                vutils.save_image(x_bar.data,
                                  '%s/x_bar_epoch_%03d_%04d.png' % (outf_path, epoch, i),normalize=True)
                vutils.save_image(x_bar_bar.data,
                                  '%s/x_bar_bar_epoch_%03d_%04d.png' % (outf_path, epoch, i),normalize=True)
                vutils.save_image(x1.data,
                        '%s/x1_epoch_%03d_%04d.png' % (outf_path, epoch, i),normalize=True)
                vutils.save_image(x2.data,
                        '%s/x2_epoch_%03d_%04d.png' % (outf_path, epoch, i),normalize=True)

                torch.save(G_xvz.state_dict(), '%s/netG_xvz_epoch_%d_%d.pth' % (outf_path, epoch, i))
                torch.save(G_vzx.state_dict(), '%s/netG_vzx_epoch_%d_%d.pth' % (outf_path, epoch, i))
                torch.save(D_xvs.state_dict(), '%s/netD_xvs_epoch_%d_%d.pth' % (outf_path, epoch, i))
